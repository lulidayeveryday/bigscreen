...
function firstShow() {
    //收藏数据的id
    var scId=["checkbox-1-4","checkbox-1-14"];
    sel_a=scId;
}

function provinceSel() {
    $('.qualityMonitoring-type').removeClass('city-select');
    $('#salesVolumeType').html("移网")
}

function citySel() {
    $('.qualityMonitoring-type').addClass('city-select');
    $('#salesVolumeType').html("宽带")
}

function searchClick() {
    //输入框内容
    var search=$("#salesVolumeSearch").val();
    //是否只查看搜藏
    var sc=$('#checkbox-sc').is(":checked");
    //当天或者累计
    var mid=$("input[name='radio-1-set']:checked").attr("id");
    if (mid=="radio-1-1"){
        //当天
    }else {
        //累计
